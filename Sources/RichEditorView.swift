...
open class RichEditorView: UIView, UIScrollViewDelegate, WKNavigationDelegate, UIGestureRecognizerDelegate {
    ...
    open func setIsEditingEnabled(_ b: Bool){
        setIsContentEditable(b)
    }
    
    open func getIsEditingEnabled(_ call: @escaping (Bool)->Void){
        getIsContentEditable { (b) in
            call(b)
        }
    }
    ...
    fileprivate func clientHeight(_ call: @escaping (Int)->Void){
        runJS("document.getElementById('editor').clientHeight;"){ d in
...
//        webView.dataDetectorTypes = UIDataDetectorTypes()
    ...
    public func getHtml(_ call: @escaping (String)->Void){
        runJS("RE.getHtml();"){ d in
            call(d)
        }
    }
    ...
    public func getText(_ call: @escaping (String)->Void){
        runJS("RE.getText()"){ d in
            call(d)
        }
    }
    ...
    public func getSelectedHref(_ call: @escaping (String?)->Void) {
        hasRangeSelection{ hasRangeSelection in
            if !hasRangeSelection { return call(nil) }
            self.runJS("RE.getSelectedHref();"){ d in
                if d == ""{
                    call(nil)
                }else{
                    call(d)
    ...
    public func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        ...
        if navigationAction.navigationType == .linkActivated{
            ...
            {
                policy =  shouldInteract ? .allow : .cancel
            }
        }
    ...
    fileprivate func setIsContentEditable(_ b: Bool){
        editingEnabledVar = b
        if isEditorLoaded {
    ...
    fileprivate func getIsContentEditable(_ call: @escaping (Bool)->Void) {
        if isEditorLoaded {
            runJS("RE.editor.isContentEditable"){ d in
                let value = d as NSString
                self.editingEnabledVar = value.boolValue
                call(self.editingEnabledVar)
            }
            
        }else{
            call(editingEnabledVar)
        }
    }
    ...
    fileprivate func relativeCaretYPosition(_ call: @escaping (Int)->Void) {
        runJS("RE.getRelativeCaretYPosition();"){ d in
            call((d as NSString).integerValue)
        }
    }

    fileprivate func updateHeight() {
        runJS("document.getElementById('editor').clientHeight;"){ heightStr in
    ...
    fileprivate func scrollCaretToVisible() {
        ...
        self.clientHeight { (clientHeight) in
            ...
            self.relativeCaretYPosition({ (caretPosition) in
